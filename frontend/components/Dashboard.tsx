...
const Dashboard: React.FC = () => {
    ...
    // Filter tasks relevant to the user's role
    const myTasks = useMemo(() => {
        if (!currentUser) return [];
        // If admin, show everything. If role based, filter by step owner.
        return batches.filter(batch => {
            const step = workflowConfig[batch.currentStepId];
            if (!step) return true; // show if step not found
            return currentUser.role === 'Admin' || currentUser.role === 'Proprietario' || currentUser.role === 'Gerente' || step.ownerRole === currentUser.role;
        });
    ...
    const stats = useMemo(() => {
        ...
        myTasks.forEach(batch => {
            const step = workflowConfig[batch.currentStepId];
            if (!step || !step.sla) { today++; return; }
            const lastUpdate = new Date(batch.lastUpdated || batch.createdAt || now);
            // Simple simulation: if last update > SLA days ago -> Delayed
            const daysSinceUpdate = (now.getTime() - lastUpdate.getTime()) / (1000 * 3600 * 24);

            if (step.sla > 0 && daysSinceUpdate > step.sla) {
                delayed++;
            } else {
                today++;
            }
                                ...
                                <div key={batch.id} className="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 flex items-center justify-between transition-colors group">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="text-right hidden md:block">
                                            ...
                                            <p className="text-sm font-medium text-slate-600 dark:text-slate-300">{new Date(batch.lastUpdated).toLocaleDateString()}</p>
                                        </div>
                                        <button
