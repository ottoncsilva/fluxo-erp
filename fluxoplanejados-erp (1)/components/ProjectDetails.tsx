...
const ProjectDetails: React.FC<ProjectDetailsProps> = ({ onBack }) => {
  const { currentProjectId, getProjectById, batches, workflowConfig, addNote, advanceBatch, currentUser, updateEnvironmentDetails, updateProjectITPP, updateClientData, origins, isLastStep } = useProjects();
        ...
        <div className="flex gap-4 items-center">
            <div>
                <div className="flex items-center gap-3">
                    ...
                    <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold uppercase ${project.client.status === 'Ativo' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
          ...
          {activeTab === 'OVERVIEW' && (
              <div className="space-y-6">
                  {projectBatches.map(batch => {
                          ...
                          const isFinished = isLastStep(batch.currentStepId);
                          ...
                          if (!step) {
                              return <div key={batch.id} className="text-red-500 bg-red-50 p-4 rounded border border-red-200">
                                  ...
                                  <button onClick={() => handleAdvance(batch.id)} className="mt-2 text-xs bg-red-100 text-red-700 px-2 py-1 rounded hover:bg-red-200">Tentar Corrigir (Avançar)</button>
                              </div>;
                          }
                          ...
                          return (
                            <div key={batch.id} className="flex flex-col gap-4">
                                ...
                                <div className={`rounded-2xl shadow-lg overflow-hidden border ${isFinished ? 'bg-white dark:bg-[#1a2632] border-green-200 dark:border-green-800' : 'bg-white dark:bg-[#1a2632] border-primary/20'}`}>
                                    <div className={`p-6 border-b flex justify-between items-center ${isFinished ? 'bg-green-50 dark:bg-green-900/20 border-green-100 dark:border-green-800' : 'bg-primary/5 border-primary/10'}`}>
                                        <div className="flex items-center gap-3">
                                            <div className={`size-12 rounded-full flex items-center justify-center shadow-lg ${isFinished ? 'bg-green-500 text-white shadow-green-500/30' : 'bg-primary text-white shadow-primary/30'}`}>
                                                <span className="material-symbols-outlined text-2xl">{isFinished ? 'celebration' : 'play_arrow'}</span>
                                            </div>
                                            <div>
                                                <p className={`text-xs font-bold uppercase tracking-wider ${isFinished ? 'text-green-600' : 'text-primary'}`}>
                                                    {isFinished ? 'Status Final' : 'Tarefa Atual'}
                                                </p>
                                        ...
                                        <div className="text-center max-w-lg">
                                            {isFinished ? (
                                                ...
                                                <>
                                                    ...
                                                    <p className="text-sm text-slate-400 mt-2">Certifique-se de que todas as atividades da etapa foram concluídas antes de avançar.</p>
                                                </>
                                            )}
                                        </div>
                              ...
                              <tr key={env.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                  ...
                                  <td className="px-6 py-4">
                                      <select 
                                          ...
                                          className={`px-2 py-1 rounded text-xs font-bold border-none cursor-pointer focus:ring-2 focus:ring-primary/50
                                  ...
                                  <td className="px-6 py-4">
                                      <input 
                                          ...
                                          className="bg-transparent border border-slate-200 dark:border-slate-700 rounded px-2 py-1 text-sm font-mono text-slate-700 dark:text-slate-300 w-32 focus:border-primary focus:ring-1 focus:ring-primary"
                                          placeholder="0.00"
                                      />
                                  </td>
                                  <td className="px-6 py-4">
                                      <div className="flex items-center gap-2">
                                          ...
                                          <input 
                                              ...
                                              className="bg-transparent border border-slate-200 dark:border-slate-700 rounded px-2 py-1 text-sm font-bold text-slate-700 dark:text-slate-300 w-16 focus:border-primary focus:ring-1 focus:ring-primary"
                                          />
                                      </div>
                                  </td>
                              </tr>
